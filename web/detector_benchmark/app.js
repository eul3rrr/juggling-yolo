...
function matches(row){
  ...
  return (source==='all'||row.selection===source)&&(kind==='all'||row.provenance.some(p=>p.event_kind===kind));
}
...
function render(){
  ...
  image.onload=()=>{draw($('baseline'),image,row.detections.baseline,'#55e58a');draw($('finetuned'),image,row.detections.finetuned,'#63edff');};
...
fetch('/api/data').then(response=>response.json()).then(value=>{data=value;refilter();}).catch(error=>{$('meta').textContent=error.message;});
