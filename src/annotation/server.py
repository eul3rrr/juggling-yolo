...
def make_server(store,host='127.0.0.1',port=43128):
    ...
    def jpeg(iid,frame,crop):
        ...
        if crop:
            x1,y1,x2,y2=item['crop'];image=image[y1:y2,x1:x2]
        ...
        def do_GET(self):
            try:
                ...
                if parsed.path=='/api/items':
                    ...
                    self.send(dict(items=items,counts=counts));return
                if parsed.path=='/api/item':
                    self.send(store.get(q['id'][0]));return
                if parsed.path=='/frame':
                    iid=q['id'][0];frame=int(q['frame'][0]);crop=q.get('crop',['0'])[0]=='1'
                    self.send(jpeg(iid,frame,crop),'image/jpeg');return
                assets={'/':'index.html','/app.js':'app.js','/styles.css':'styles.css'}
                if parsed.path in assets:
                    path=WEB/assets[parsed.path]
                    self.send(path.read_bytes(),mimetypes.guess_type(path)[0] or 'text/plain');return
                self.send({'error':'not found'},status=404)
            except (ValueError,KeyError,OSError) as e: self.send({'error':str(e)},status=400)

        def do_POST(self):
            origin=self.headers.get('Origin')
            if origin and origin != 'http://'+self.headers.get('Host',''):
                self.send({'error':'Cross-origin write refused'},status=403);return
            ...
            try:
                ...
                if self.path!='/api/save': self.send({'error':'not found'},status=404);return
