...
SURVEY = dict(hand_overlap=['none','slight','moderate','heavy'],visibility=['clear','partial','tiny_fragment'],
              motion_blur=['none','mild','strong'],annotation_confidence=['certain','uncertain'])
...
class Store:
        ...
        with self.connection() as c:
            ...
            for item in candidates:
                ...
                if c.execute('SELECT 1 FROM items WHERE id=?',(iid,)).fetchone():
                    continue # never resurrect deleted predictions or overwrite human work
                data={k:v for k,v in item.items() if k!='boxes'}
                data.update(focus_status='not_applicable' if item['priority']==2 else '',focus_box_id=None,
                            notes='',all_visible_confirmed=False)
                c.execute('INSERT INTO items(id,source_id,frame,priority,data) VALUES (?,?,?,?,?)',
                          (iid,sid,item['frame'],item['priority'],encoded(data)))
                for index,pred in enumerate(detections.get(item['frame'],[])):
                    ...
                    if not all(math.isfinite(v) for v in coords) or not (0<=x1<x2<=source['width'] and 0<=y1<y2<=source['height']):
                        continue # retain malformed prediction in audit, not an editable box
                    b=dict(id=pid,x1=x1,y1=y1,x2=x2,y2=y2,class_name='juggling_ball',annotation_source='preannotation',
                           prediction_id=pid,source_detector_box=coords,source_detector_confidence=pred['confidence'],
                           **{key:'' for key in SURVEY})
    ...
    def get(self, iid, connection=None):
        if connection is None:
            with self.connection() as c: return self.get(iid,c)
        c=connection
        row=c.execute('SELECT * FROM items WHERE id=?',(iid,)).fetchone()
        if row is None: raise ValueError('Unknown annotation item')
    ...
    def save(self, iid, payload):
        with self.connection() as c:
            c.execute('BEGIN IMMEDIATE')
            old=self.get(iid,c)
            if payload.get('revision') != old['revision']: raise ValueError('Stale revision: reload before saving')
            status=payload.get('status')
            if status not in ('pending','completed','skipped'): raise ValueError('Invalid status')
            focus=payload.get('focus_status','')
            allowed=('not_applicable',) if old['priority']==2 else ('','visible','no_visible_evidence','uncertain')
            if focus not in allowed or (status=='completed' and not focus): raise ValueError('Select focus status')
            boxes=[]; ids=set(); used_predictions=set()
            predictions={p['id']:p for p in old['predictions']}
            for raw in payload.get('boxes',[]):
                bid=raw.get('id')
                if not isinstance(bid,str) or not bid or len(bid)>100 or bid in ids: raise ValueError('Invalid/duplicate box ID')
                ids.add(bid)
                ...
                if pid and (pid not in predictions or pid in used_predictions): raise ValueError('Invalid detector prediction relation')
                if pid: used_predictions.add(pid)
                pred=predictions.get(pid)
                boxes.append(dict(id=bid,x1=x1,y1=y1,x2=x2,y2=y2,class_name='juggling_ball',**survey,
                    annotation_source='preannotation' if pred else 'manual',prediction_id=pid if pred else None,
                    source_detector_confidence=pred['confidence'] if pred else None,source_detector_box=pred['box'] if pred else None))
            focus_box=payload.get('focus_box_id') or None
            if focus_box and (focus!='visible' or focus_box not in ids): raise ValueError('Invalid focus box')
            ...
            data.update(focus_status=focus,focus_box_id=focus_box,notes=str(payload.get('notes',''))[:10000],
                        all_visible_confirmed=payload.get('all_visible_confirmed') is True)
