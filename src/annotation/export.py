...
def normalize_box(box, width, height):
    return ((box['x1'] + box['x2']) / (2 * width), (box['y1'] + box['y2']) / (2 * height), (box['x2'] - box['x1']) / width, (box['y2'] - box['y1']) / height)

def export_yolo(store, output, frame_reader=read_frame):
    output = Path(output).resolve()
    if output.exists():
        raise ValueError('Export output must not exist; choose a new snapshot directory')
    output.parent.mkdir(parents=True, exist_ok=True)
    temp = Path(tempfile.mkdtemp(prefix='.annotation-export-', dir=output.parent))
    try:
        for d in ('images/unassigned', 'labels/unassigned'):
