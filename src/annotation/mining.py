...
def load_links(path):
    ...
    with path.open(newline="") as f:
        ...
        return [dict(row, source_track_id=int(row["source_track_id"]),
                     target_track_id=int(row["target_track_id"])) for row in reader
                if row.get("accepted", "true").lower() in ("true", "1", "yes")]
...
def crop_geometry(point, width, height, pose=None):
    ...
    if pose:
        def joint(side, name):
            try:
                ...
                if conf >= .2 and math.isfinite(px+py) and 0<=px<=width and 0<=py<=height:
                    return px,py
        ...
        try:
            ...
            if not math.isfinite(scale): scale=0
        except ValueError:
            pass
    mx,my = max(width*.275,scale*.8),max(height*.275,scale*.8)
    if len(points) == 1:
    ...
    return [max(0,math.floor(min(p[0] for p in points)-mx)),
            max(0,math.floor(min(p[1] for p in points)-my)),
            min(width,math.ceil(max(p[0] for p in points)+mx)),
            min(height,math.ceil(max(p[1] for p in points)+my))]
...
def mine_candidates(tracks, links, fps, count, width, height, poses=None):
    ...
    def add(frame, reason, priority, event, point):
        ...
        if frame not in items:
            items[frame] = dict(frame=frame, timestamp=frame/fps, priority=priority,
                reasons=[],provenance=[],focus_point=list(point),
                crop=crop_geometry(point,width,height,poses.get(frame)),boxes=[])
        item = items[frame]
        if priority < item["priority"]:
            item.update(priority=priority,focus_point=list(point),crop=crop_geometry(point,width,height,poses.get(frame)))
        if reason not in item["reasons"]: item["reasons"].append(reason)
        if prov not in item["provenance"]: item["provenance"].append(prov)
    for row in sorted(links,key=lambda r:(r["source_track_id"],r["target_track_id"])):
        src,dst = row["source_track_id"],row["target_track_id"]
        if src not in tracks or dst not in tracks: raise ValueError("Link references absent track")
        a,b = tracks[src].last_observed,tracks[dst].first_observed
        if a is None or b is None or a.frame>=b.frame: raise ValueError("Link needs observed, forward boundaries")
        ...
        ev = dict(event_key=f"linked:{src}:{a.frame}:{dst}:{b.frame}",kind="linked_gap",
                  focus_track_id=src,related_track_id=dst,end_frame=a.frame,start_frame=b.frame,accepted_link=row)
    ...
    # Reuse reviewer END/orphan semantics exactly, without modifying the reviewer.
    for e in generate_events(tracks,{},fps,count):
        kind = "track_end" if e.kind=="end" else "orphan_start"
        ev = dict(event_key=e.event_key,kind=kind,focus_track_id=e.primary.track_id,
                  related_track_id=None,end_frame=e.primary_end_frame if kind=="track_end" else None,
                  start_frame=e.primary_end_frame if kind=="orphan_start" else None,accepted_link=None)
    ...
    margin = max(1,round(.25*fps))
    eligible = {}
    for tid,tr in sorted(tracks.items()):
        observed = {o.frame:o for o in tr.observed}
        for f,o in sorted(observed.items()):
            if f in items or not 0<=f<count or any(abs(f-b)<=margin for b in boundaries): continue
            if all(t in observed for t in range(f-margin,f+margin+1)):
                eligible.setdefault(f,(tid,o))
    # At most one ordinary frame per quarter second, evenly across available runs.
    spaced=[]
    for f in sorted(eligible):
        if not spaced or f-spaced[-1]>=margin: spaced.append(f)
    target = min(len(spaced),max(1,round(len(items)*.3/.7)))
    chosen = sorted({spaced[round(i*(len(spaced)-1)/max(1,target-1))] for i in range(target)})
    for f in chosen:
        tid,o=eligible[f]
        add(f,"ordinary",2,dict(event_key=f"ordinary:{tid}:{f}",kind="ordinary",focus_track_id=None,
            related_track_id=None,end_frame=None,start_frame=None,accepted_link=None),(o.center_x,o.center_y))
...
def gap_frames(end, start, fps):
    if start - end - 1 <= round(.2 * fps):
        return list(range(end + 1, start))
    near = max(1, round(fps / 60))
    return sorted({t for t in [end+near,end+2*near,start-2*near,start-near,
                   *(round(end+(start-end)*q) for q in (.25,.5,.75))]
                   if end < t < start})
