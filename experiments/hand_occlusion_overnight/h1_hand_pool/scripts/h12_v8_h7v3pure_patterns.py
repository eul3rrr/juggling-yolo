#!/usr/bin/env python3
"""H12 v8 - per-frame juggling pattern inference on H7v3pure chains
with H10 v9 quality.

H12 v8 was the best event-log-based pattern classifier that uses
h7v2 chains, but it
suffered on YouTube because H10 v5 over-counted chains (n_total=5
in 601/898 frames). H7v3pure (h7v2 + H15v2 V-shape reclassification) is the new
chain pipeline. H10v9 is the new chain quality score.

H12 v8 hypothesis: re-running the v7 pattern inference on H7v3pure
chains with H10 v9 quality should give:
  * identical: chain 30 quality 0.427 -> 0.727 (now CONFIDENT),
    4 new V-reclassified catch+throw events on chains 13, 20, 24, 30
  * YouTube: 1 new V-reclassified (FP) catch+throw on chain 12
"""
...
def build_per_frame_census(stem: str) -> dict[int, dict]:
    """Build per-frame census from h7v3pure chains.
...
def main():
    summary = {"videos": {}}
    for stem in STEMS:
        print(f"\n=== {stem} (H12 v8: H7v3pure chains + H10 v9 quality) ===")
        ...
        out_csv = H1_DATA / f"pattern_inference_v8_{stem}.csv"
        with out_csv.open("w", newline="") as fh:
            w = csv.DictWriter(fh, fieldnames=list(results[0].keys()))
            w.writeheader()
            w.writerows(results)
        print(f"  wrote: {out_csv.name}")
        out_phases = H1_DATA / f"pattern_phases_v8_{stem}.csv"
        ...
        out_events = H1_DATA / f"catch_throw_timeline_v8_{stem}.csv"
    ...
    out = H1_DATA / "h12_v8_summary.json"
