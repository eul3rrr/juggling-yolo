#!/usr/bin/env python3
"""H22 v2: H10 v9 chain quality on H7v3veto chains."""
...
def load_chains(stem: str, prefix: str = "h7v3veto") -> list[dict]:
    out = []
...
def load_edges(stem: str, prefix: str = "h7v3veto") -> list[dict]:
    out = []
...
def main():
    ...
    for stem, video_key in STEMS.items():
        ...
        h7v3veto_chains = load_chains(stem, "h7v3veto")
        h7v3pure_edges = load_edges(stem, "h7v3pure")
        h7v3veto_edges = load_edges(stem, "h7v3veto")
        print(f"  h7v3pure: {len(h7v3pure_chains)} chains, {len(h7v3pure_edges)} edges")
        print(f"  h7v3veto: {len(h7v3veto_chains)} chains, {len(h7v3veto_edges)} edges")
        ...
        h9_cov = compute_h9_for_chains(stem, h7v3veto_chains)

        chain_edges = defaultdict(list)
        for c in h7v3veto_chains:
            tids = c["tids"]
            for i in range(len(tids) - 1):
                a, b = tids[i], tids[i + 1]
                for e in h7v3veto_edges:
        ...
        results.sort(key=lambda r: -r["quality_v11"])
        print(f"  Top 3 chains by v11 quality:")
        for r in results[:3]:
    ...
    out_path = H1_DATA / "h22v2_chain_quality_summary.json"
    out_path.write_text(json.dumps(summary, indent=2, default=str))
    print(f"\nSaved: {out_path}")

    for stem in STEMS:
        v = summary["videos"][stem]
        with (H1_DATA / f"h22v2_chain_quality_{stem}.csv").open("w", newline="") as fh:
            w = csv.DictWriter(fh, fieldnames=[
                "chain_id", "n_tracklets", "n_hand_edges", "n_v_reclass_edges",
                "n_air_edges", "n_h3_confirmed", "h3_score", "h8_score",
                "h9_score", "h8v8_score", "quality_v11",
            ])
            w.writeheader()
            for r in v["results"]:
