#!/usr/bin/env python3
"""
H94 v4 — H74v4 + H87+max_aloft guard + H43/H69 pct_ge1 guard.

Background
==========
H94 v3 (H74v4 + H43/H69 pct_ge1 guard + H87) achieves 90.5% accuracy
on H93 corrected GT with 1 FP and 1 FN.

The remaining error is f=733-766 (CASCADE_3+ JUGGLING per H93 multi-rater)
which H87 wrongly rejects (pct_ge3=0.00 < 0.20).

Key insight: f=733-766 has max_aloft=1 (only 1 ball ever aloft — real
3-ball cascade with 1 ball in air at a time). A genuine static hold
with similar H40v2 signature (e.g., f=685-716) has max_aloft=4 due to
manipulation/roll motions producing multiple YOLO detections.

Hypothesis
==========
A `max_aloft >= 2` guard on H87 prevents the false reject on f=733-766
while still catching the real STATIC_HOLD f=685-716.
...
Method
======
1. Load H40v2 + H70 + H78 + H90 phase features (with max_aloft)
2. Test H94 v4 on H93 corrected GT (21 phases)
3. Sensitivity grid: max_aloft threshold ∈ {1, 2, 3, 4}
4. Cross-validate on the 113 manual review pairs (H59 GT)
"""
...
def h94_v4_decision(pattern, conf, spec_conc, h74_sig, mean_diff, aloft,
...
def main():
    ...
    print("=" * 80)
    print("H94 v4 — H74v4 + H87+max_aloft guard + H43/H69 pct_ge1 guard")
    print("=" * 80)

    # Per-phase features
    print("\nPer-phase features (max_aloft is the new discriminator):")
    print(f"{'phase':<35} {'verdict':<22} {'c00g1':>5} {'c00g3':>5} {'max_aloft':>9} {'var':>6} {'uLR':>3}")
    for key in sorted(CORRECTED_GT.keys()):
        ...
        print(f"{label:<35} {verdict:<22} {sig['pct_ge1']:>5.2f} {sig['pct_ge3']:>5.2f} {sig['max_aloft']:>9} {h74['var']:>6.3f} {h74['unique_LR']:>3}")

    # Sensitivity grid 2D: max_aloft × pct_ge1
    print("\n=== H94 v4 2D sensitivity grid (max_aloft_thr × pct_ge1_thr) ===")
    ...
    for max_a in [1, 2, 3, 4]:
        for pct_g in [0.80, 0.85, 0.88, 0.90, 0.92, 0.95, 1.00]:
            r = evaluate(CORRECTED_GT, all_signals, h74_signals, h78, aloft_signals,
                         h94_v4_decision, "H94 v4", pct_ge1_thr=pct_g, max_aloft_thr=max_a)
            c = r["combined"]
            mark = ""
            if c[0] == 17 and c[1] == 4 and c[2] == 0 and c[3] == 0:
                mark = " <-- 100% acc"
            elif c[0] == 16 and c[1] == 5 and c[2] == 0 and c[3] == 0:
                mark = " <-- PERFECT (corrected GT)"
            elif c[0] == 17 and c[1] == 4 and c[2] == 0 and c[3] == 0:
                mark = " <-- PERFECT (incl. OTHER_CROSSED_ARM)"
            print(f"{max_a:>10} {pct_g:>8.2f} {c[0]:>3} {c[1]:>3} {c[2]:>3} {c[3]:>3} {c[4]:>6.3f} {c[5]:>6.3f} {c[6]:>6.3f}{mark}")

    # Show per-phase for chosen threshold
    chosen = (2, 0.92)
    print(f"\n=== H94 v4 per-phase (max_aloft={chosen[0]}, pct_ge1={chosen[1]}) ===")
    r = evaluate(CORRECTED_GT, all_signals, h74_signals, h78, aloft_signals,
                 h94_v4_decision, "H94 v4", pct_ge1_thr=chosen[1], max_aloft_thr=chosen[0])
    c = r["combined"]
    i = r["ident"]
    y = r["youtu"]
    ...
    # Save summary
    summary = {
        "H94_v4_methodology": "H74v4 + H87+max_aloft guard + H43/H69 pct_ge1 guard",
        "sensitivity_grid": {},
    ...
    with open(f"{H1_DATA}/h94_v4_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\nWrote {H1_DATA}/h94_v4_summary.json")
